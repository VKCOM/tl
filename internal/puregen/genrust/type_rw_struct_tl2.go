...
func (trw *TypeRWStruct) calculateLayoutCall(
	...
	if trw.isUnwrapType() {
		return trw.Fields[0].t.CalculateLayoutCall(directImports, bytesVersion, targetSizes, targetObject, zeroIfEmpty, refObject)
	}
	...
	if trw.isAlias() && trw.unionParent == nil {
		actualType := trw.Fields[0].t.TypeString2(bytesVersion, directImports, false, false)
		if refObject {
...
func (trw *TypeRWStruct) writeTL2Call(
	...
	if trw.isUnwrapType() {
		return trw.Fields[0].t.WriteTL2Call(directImports, bytesVersion, targetSizes, targetBytes, targetObject, zeroIfEmpty, refObject)
	}
	if trw.wr.IsTrueType() && trw.unionParent == nil {
		if zeroIfEmpty {
			return "if false {"
...
func (trw *TypeRWStruct) readTL2Call(
	...
	if trw.isUnwrapType() {
		return trw.Fields[0].t.ReadTL2Call(directImports, bytesVersion, targetBytes, targetObject, canDependOnLocalBit, refObject)
	}
	additionalSuffix := ""
...
func (trw *TypeRWStruct) skipTL2Call(
	...
	if trw.isUnwrapType() {
		return trw.Fields[0].t.SkipTL2Call(directImports, bytesVersion, targetBytes, canDependOnLocalBit, refObject)
	}
