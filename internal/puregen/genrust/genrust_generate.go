...
func (gen *genRust) generateTypePrimitive(myWrapper *TypeRWWrapper, pureType pure.TypeInstance) error {
...
func (gen *genRust) generateTypeStruct(myWrapper *TypeRWWrapper, pureType *pure.TypeInstanceStruct,
...
func (gen *genRust) generateTypeBool(myWrapper *TypeRWWrapper, pureType *pure.TypeInstancePrimitive, isBit bool) error {
	head, tail := myWrapper.resolvedT2GoName("")
	myWrapper.goGlobalName = gen.globalDec.DeconflictName(head + tail)
	head, tail = myWrapper.resolvedT2GoName(myWrapper.ns.name)
	myWrapper.goLocalName = myWrapper.ns.decGo.DeconflictName(head + tail)
...
func (gen *genRust) generateTypeUnion(myWrapper *TypeRWWrapper, pureType *pure.TypeInstanceUnion) error {
...
func (gen *genRust) GenerateTypeArray(myWrapper *TypeRWWrapper, pureType *pure.TypeInstanceArray) error {
...
func (gen *genRust) GenerateTypeDict(myWrapper *TypeRWWrapper, pureType *pure.TypeInstanceDict) error {
	field := pureType.Field()
	fieldType, err := gen.getType(field.TypeInstance())
	if err != nil {
		return err
...
func (gen *genRust) generateNamespacesCode(anyTypeAlias bool, anyFunctions bool, name string, namespace *Namespace, sortedImports []string, directImports *DirectImports) string {
	return ""
}
