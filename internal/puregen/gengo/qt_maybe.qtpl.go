...
`)
    ...
    if basictl.RandomUint(rg) & 1 == 1 {
        item.Ok = true
        `)
		qw422016.N().S(maybe.element.t.TypeRandomCode(bytesVersion, directImports, maybe.wr.ins, "item.Value", formatNatArgs(nil, maybe.ElementNatArgs()), false))
		qw422016.N().S(`
    } else {
        item.Ok = false
    }
...
func (item *`)
		...
		qw422016.N().S(`) {
    `)
		qw422016.N().S(maybe.element.t.TypeRepairMasksCode(bytesVersion, directImports, maybe.wr.ins, "item.Value", formatNatArgs(nil, maybe.ElementNatArgs()), false))
		qw422016.N().S(`
}
`)
	}
...
`)
    ...
    if item.Ok {
        `)
		qw422016.N().S(maybe.element.t.TypeReadingCode(bytesVersion, directImports, maybe.wr.ins, "item.Value", maybe.element.Bare(), formatNatArgs(nil, maybe.ElementNatArgs()), false, true))
		qw422016.N().S(`
    }
    return w, nil
`)
...
`)
    ...
    if item.Ok {
		...
		qw422016.N().S(`)
        `)
		qw422016.N().S(maybe.element.t.TypeWritingCode(bytesVersion, directImports, maybe.wr.ins, "item.Value", maybe.element.Bare(), formatNatArgs(nil, maybe.ElementNatArgs()), false, true, writeElementNeedsError))
		qw422016.N().S(`
    }
`)
		if writeElementNeedsError {
...
`)
  ...
  if _ok {
    ...
    `)
	qw422016.N().S(maybe.element.t.TypeJSON2ReadingCode(bytesVersion, directImports, maybe.wr.ins, "in2Pointer", "item.Value", formatNatArgs(nil, maybe.ElementNatArgs()), false))
	qw422016.N().S(`
  }
  return nil
}
...
`)
	...
	qw422016.N().S(`...)
    `)
	qw422016.N().S(maybe.element.t.TypeJSONWritingCode(bytesVersion, directImports, maybe.wr.ins, "item.Value", formatNatArgs(nil, maybe.ElementNatArgs()), false, writeElementNeedsError))
	qw422016.N().S(`
`)
	if emptyCondition != "" {
		qw422016.N().S(`    }
