...
func (gen *genGo) generateCodeGolang() error {
	...
	if !gen.options.Go.SplitInternal {
		...
		printSortedNamespaces := func() {
			...
			//	fmt.Printf("recursive import namespace %s\n", ins)
			// }
		}
		nextDebugID := 0
		for _, v := range sortedTypes { // start with each type in its own internal namespace
		...
		for i := 0; i < len(internalNamespaces); {
			...
			if importMe, ok := recursiveImports[from]; ok && len(importMe) != 0 {
				into := importMe[0]
				// if options.Verbose {
				// fmt.Printf("----merging cycle namespace %sinto  %s (total %d)\n", from, into, len(internalNamespaces))
				// }
...
`)
	...
	for name, namespace := range gen.Namespaces {
		...
		if gen.options.Go.SplitInternal {
			hasTypes := false
			metaCode := gen.generateMetaInit(true, namespace.types, func(ns string) bool {
				return true
			}, &hasTypes, gen.GlobalPackageName+name)
			if hasTypes {
				if err := outdir.AddCodeFile(filepath.Join(gen.GlobalPackageName+name, "metamini.go"), gen.options.CopyrightText+metaCode); err != nil {
					return err
				}
			}
		}
