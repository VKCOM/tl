...
func (item *CasesTL2TestFunctionWithDep1) Write(w []byte) []byte {
...
func (item *CasesTL2TestFunctionWithDep1) WriteBoxed(w []byte) []byte {
	panic(basictl.TL2Error("not implemented for tl2 type"))
}
...
func (item *CasesTL2TestFunctionWithDep1) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.N != 0 {
...
func (item *CasesTL2TestFunctionWithDep1) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, internal.ErrorInvalidUnionIndex("casesTL2.testFunctionWithDep1", index)
		}
	}
	if block&2 != 0 {
