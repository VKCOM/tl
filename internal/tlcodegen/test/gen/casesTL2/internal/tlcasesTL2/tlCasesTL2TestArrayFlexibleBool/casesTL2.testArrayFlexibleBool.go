...
func (CasesTL2TestArrayFlexibleBool) TLTag() uint32  { return 0x00000000 }
...
func (item *CasesTL2TestArrayFlexibleBool) FillRandom(rg *basictl.RandGenerator) {
	...
	tlBuiltinVectorBool.BuiltinVectorBoolFillRandom(rg, &item.Arr)
}
...
func (item *CasesTL2TestArrayFlexibleBool) Write(w []byte) []byte {
	return item.WriteTL1(w)
}
func (item *CasesTL2TestArrayFlexibleBool) WriteTL1(w []byte) []byte {
...
func (item *CasesTL2TestArrayFlexibleBool) WriteBoxed(w []byte) []byte {
	return item.WriteTL1Boxed(w)
}
func (item *CasesTL2TestArrayFlexibleBool) WriteTL1Boxed(w []byte) []byte {
	panic(basictl.TL2Error("not implemented for tl2 type"))
}
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "arr":
				...
				if err := tlBuiltinVectorBool.BuiltinVectorBoolReadJSONGeneral(tctx, in, &item.Arr); err != nil {
					return err
				}
...
func (item *CasesTL2TestArrayFlexibleBool) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	if len(item.Arr) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"arr":`...)
		w = tlBuiltinVectorBool.BuiltinVectorBoolWriteJSONOpt(tctx, w, item.Arr)
	}
	return append(w, '}')
}
...
func (item *CasesTL2TestArrayFlexibleBool) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinVectorBool.BuiltinVectorBoolCalculateLayout(sizes, true, &item.Arr); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTL2TestArrayFlexibleBool) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = tlBuiltinVectorBool.BuiltinVectorBoolInternalWriteTL2(w, sizes, true, &item.Arr); sz != 0 {
		currentBlock |= 4
	}
...
func (item *CasesTL2TestArrayFlexibleBool) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&4 != 0 {
		if currentR, err = tlBuiltinVectorBool.BuiltinVectorBoolInternalReadTL2(currentR, &item.Arr); err != nil {
			return currentR, err
		}
	} else {
		item.Arr = item.Arr[:0]
