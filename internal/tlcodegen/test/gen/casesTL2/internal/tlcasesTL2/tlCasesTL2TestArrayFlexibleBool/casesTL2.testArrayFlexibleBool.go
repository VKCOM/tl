...
func (item *CasesTL2TestArrayFlexibleBool) CalculateLayout(sizes []int) []int {
	...
	if len(item.Arr) != 0 {
		sizes = tlBuiltinVectorBool.BuiltinVectorBoolCalculateLayout(sizes, &item.Arr)
...
func (item *CasesTL2TestArrayFlexibleBool) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.Arr) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = tlBuiltinVectorBool.BuiltinVectorBoolInternalWriteTL2(w, sizes, &item.Arr)
		} else {
			sizes = sizes[1:]
		}
	}
