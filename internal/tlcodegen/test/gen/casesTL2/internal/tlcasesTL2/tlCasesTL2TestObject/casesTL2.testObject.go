...
type CasesTL2TestObject struct {
	...
	F6       []tlBenchmarksVrutoyTopLevelUnion.BenchmarksVrutoyTopLevelUnion
	F7       []tlTrue.True // Optional, use SetF7
	tl2mask0 byte
}
...
func (item *CasesTL2TestObject) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "ns":
				if propNsPresented {
					return internal.ErrorInvalidJSONWithDuplicatingKeys("casesTL2.testObject", "ns")
				}
...
func (item *CasesTL2TestObject) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if item.tl2mask0&4 != 0 {
		sizes, sz = tlBuiltinVectorTrue.BuiltinVectorTrueCalculateLayout(sizes, false, &item.F7)
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTL2TestObject) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if item.tl2mask0&4 != 0 {
		w, sizes, _ = tlBuiltinVectorTrue.BuiltinVectorTrueInternalWriteTL2(w, sizes, false, &item.F7)
		currentBlock |= 2
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
