	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "f4":
				...
				if err := item.F4.ReadJSONGeneral(jctx, in); err != nil {
					return err
				}
			case "f5":
				if propF5Presented {
			...
			case "f7":
				...
				if err := BuiltinVectorTrueReadJSONGeneral(jctx, in, &item.F7); err != nil {
					return err
				}
				item.tl2mask0 |= 4
			default:
...
func (item *CasesTL2TestObject) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.N != 0 {
	...
	if item.tl2mask0&4 != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"f7":`...)
		w = BuiltinVectorTrueWriteJSONOpt(jctx, w, item.F7)
	}
	return append(w, '}')
}
...
func (item *CasesTL2TestObject) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesTL2TestObject) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
