...
func (CasesTestInplaceStructArgs) TLTag() uint32  { return 0x00000000 }
...
func (item *CasesTestInplaceStructArgs) WriteTL1(w []byte) []byte {
...
func (item *CasesTestInplaceStructArgs) WriteTL1Boxed(w []byte) []byte {
	panic(basictl.TL2Error("not implemented for tl2 type"))
}
...
func (item *CasesTestInplaceStructArgs) WriteJSON(w []byte) []byte {
	return item.WriteJSONOpt(nil, w)
}
func (item *CasesTestInplaceStructArgs) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.A1 != 0 {
