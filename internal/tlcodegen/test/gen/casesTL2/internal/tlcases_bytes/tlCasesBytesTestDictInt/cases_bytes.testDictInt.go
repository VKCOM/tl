...
import (
	...
	"github.com/vkcom/tl/internal/tlcodegen/test/gen/casesTL2/internal/tl/tlBuiltinDictIntInt"
...
func (item *CasesBytesTestDictInt) Reset() {
	tlBuiltinDictIntInt.BuiltinDictIntIntReset(item.Dict)
}

func (item *CasesBytesTestDictInt) FillRandom(rg *basictl.RandGenerator) {
	tlBuiltinDictIntInt.BuiltinDictIntIntFillRandom(rg, &item.Dict)
}
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "dict":
				...
				if err := tlBuiltinDictIntInt.BuiltinDictIntIntReadJSONGeneral(tctx, in, &item.Dict); err != nil {
					return err
				}
...
func (item *CasesBytesTestDictInt) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	if len(item.Dict) != 0 {
		...
		w = append(w, `"dict":`...)
		w = tlBuiltinDictIntInt.BuiltinDictIntIntWriteJSONOpt(tctx, w, item.Dict)
	}
	return append(w, '}')
}
...
func (item *CasesBytesTestDictInt) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinDictIntInt.BuiltinDictIntIntCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
...
func (item *CasesBytesTestDictInt) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if w, sizes, sz = tlBuiltinDictIntInt.BuiltinDictIntIntInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
...
func (item *CasesBytesTestDictInt) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = tlBuiltinDictIntInt.BuiltinDictIntIntInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		tlBuiltinDictIntInt.BuiltinDictIntIntReset(item.Dict)
...
type CasesBytesTestDictIntBytes struct {
	Dict []tlDictFieldIntInt.DictFieldIntInt
}
...
func (item *CasesBytesTestDictIntBytes) FillRandom(rg *basictl.RandGenerator) {
	tlBuiltinDictIntInt.BuiltinDictIntIntBytesFillRandom(rg, &item.Dict)
}
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "dict":
				...
				if err := tlBuiltinDictIntInt.BuiltinDictIntIntBytesReadJSONGeneral(tctx, in, &item.Dict); err != nil {
					return err
				}
				propDictPresented = true
			default:
...
func (item *CasesBytesTestDictIntBytes) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	if len(item.Dict) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"dict":`...)
		w = tlBuiltinDictIntInt.BuiltinDictIntIntBytesWriteJSONOpt(tctx, w, item.Dict)
	}
	return append(w, '}')
}
...
func (item *CasesBytesTestDictIntBytes) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinDictIntInt.BuiltinDictIntIntBytesCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesBytesTestDictIntBytes) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	currentBlockPosition := len(w)
	w = append(w, 0)
	if w, sizes, sz = tlBuiltinDictIntInt.BuiltinDictIntIntBytesInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
...
func (item *CasesBytesTestDictIntBytes) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = tlBuiltinDictIntInt.BuiltinDictIntIntBytesInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		item.Dict = item.Dict[:0]
