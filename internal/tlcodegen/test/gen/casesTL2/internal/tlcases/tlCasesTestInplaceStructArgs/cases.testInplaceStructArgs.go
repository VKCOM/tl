...
func (item *CasesTestInplaceStructArgs) Write(w []byte) []byte {
...
func (item *CasesTestInplaceStructArgs) WriteBoxed(w []byte) []byte {
...
func (item *CasesTestInplaceStructArgs) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.A1 != 0 {
...
func (item *CasesTestInplaceStructArgs) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, internal.ErrorInvalidUnionIndex("cases.testInplaceStructArgs", index)
		}
	}
	if block&2 != 0 {
