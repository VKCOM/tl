...
func (item *CasesTestOutFieldMask) FillRandom(rg *basictl.RandGenerator) {
	...
	tlBuiltinVectorInt.BuiltinVectorIntFillRandom(rg, &item.F3)
}
...
func (item *CasesTestOutFieldMask) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	if len(item.F3) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"f3":`...)
		w = tlBuiltinVectorInt.BuiltinVectorIntWriteJSONOpt(tctx, w, item.F3)
	}
	return append(w, '}')
}
...
func (item *CasesTestOutFieldMask) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinVectorInt.BuiltinVectorIntCalculateLayout(sizes, true, &item.F3); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTestOutFieldMask) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = tlBuiltinVectorInt.BuiltinVectorIntInternalWriteTL2(w, sizes, true, &item.F3); sz != 0 {
		currentBlock |= 8
	}
...
func (item *CasesTestOutFieldMask) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&8 != 0 {
		if currentR, err = tlBuiltinVectorInt.BuiltinVectorIntInternalReadTL2(currentR, &item.F3); err != nil {
			return currentR, err
		}
	} else {
		item.F3 = item.F3[:0]
