	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "arr":
				...
				if err := BuiltinVectorStringReadJSONGeneral(jctx, in, &item.Arr); err != nil {
					return err
				}
			default:
...
func (item *CasesBytesTestArray) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.N != 0 {
	...
	if len(item.Arr) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"arr":`...)
		w = BuiltinVectorStringWriteJSONOpt(jctx, w, item.Arr)
	}
	return append(w, '}')
}
...
func (item *CasesBytesTestArray) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorStringCalculateLayout(sizes, true, &item.Arr); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesBytesTestArray) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = BuiltinVectorStringInternalWriteTL2(w, sizes, true, &item.Arr); sz != 0 {
		currentBlock |= 4
	}
...
func (item *CasesBytesTestArray) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesBytesTestArray) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&4 != 0 {
		if currentR, err = BuiltinVectorStringInternalReadTL2(currentR, &item.Arr); err != nil {
			return currentR, err
		}
	} else {
		item.Arr = item.Arr[:0]
...
func (item *CasesBytesTestArray) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "arr":
				...
				propArrPresented = true
				if err := BuiltinVectorStringBytesReadJSONGeneral(jctx, in, &item.Arr); err != nil {
					return err
				}
			default:
				return ErrorInvalidJSONExcessElement("cases_bytes.testArray", key)
...
func (item *CasesBytesTestArrayBytes) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	if item.N != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"n":`...)
	...
	if len(item.Arr) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"arr":`...)
		w = BuiltinVectorStringBytesWriteJSONOpt(jctx, w, item.Arr)
	}
	return append(w, '}')
}
...
func (item *CasesBytesTestArrayBytes) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorStringBytesCalculateLayout(sizes, true, &item.Arr); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesBytesTestArrayBytes) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = BuiltinVectorStringBytesInternalWriteTL2(w, sizes, true, &item.Arr); sz != 0 {
		currentBlock |= 4
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
...
func (item *CasesBytesTestArrayBytes) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
	var sizes, sizes2 []int
	if tctx != nil {
		sizes = tctx.SizeBuffer[:0]
	}
...
func (item *CasesBytesTestArrayBytes) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&4 != 0 {
		if currentR, err = BuiltinVectorStringBytesInternalReadTL2(currentR, &item.Arr); err != nil {
			return currentR, err
		}
	} else {
		item.Arr = item.Arr[:0]
...
func (item *CasesBytesTestArrayBytes) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
	return item.InternalReadTL2(r)
}
