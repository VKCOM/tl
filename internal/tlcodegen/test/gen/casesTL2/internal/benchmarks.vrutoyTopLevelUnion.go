...
var _BenchmarksVrutoyTopLevelUnion = [2]UnionElement{
	{TLTag: 0x00000000, TLName: "benchmarks.vrutoytopLevelUnionBig", TLString: ""},
	{TLTag: 0x00000000, TLName: "benchmarks.vrutoytopLevelUnionEmpty", TLString: ""},
}
...
func (item *BenchmarksVrutoyTopLevelUnion) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if (block & 1) != 0 {
		...
		if item.index >= 2 {
			return r, ErrorInvalidUnionIndex("benchmarks.VrutoyTopLevelUnion", item.index)
		}
	}
	switch item.index {
	case 0:
...
func (item *BenchmarksVrutoyTopLevelUnion) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *BenchmarksVrutoyTopLevelUnion) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) ([]byte, error) {
...
func (item *BenchmarksVrutoyTopLevelUnion) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	switch _tag {
	case "Big", "benchmarks.vrutoytopLevelUnionBig":
		item.index = 0
	...
	case "Empty", "benchmarks.vrutoytopLevelUnionEmpty":
		item.index = 1
	default:
...
func (item *BenchmarksVrutoytopLevelUnionBig) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
	var sizes, sizes2 []int
	if tctx != nil {
		sizes = tctx.SizeBuffer[:0]
	}
...
func (item *BenchmarksVrutoytopLevelUnionBig) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
	currentSize := 0
	if r, currentSize, err = basictl.TL2ParseSize(r); err != nil {
		return r, err
	}
...
func (BenchmarksVrutoytopLevelUnionEmpty) TLTag() uint32 { return 0x00000000 }
...
func (item *BenchmarksVrutoytopLevelUnionEmpty) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
	var sizes, sizes2 []int
	if tctx != nil {
		sizes = tctx.SizeBuffer[:0]
	}
...
func (item *BenchmarksVrutoytopLevelUnionEmpty) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
	currentSize := 0
	if r, currentSize, err = basictl.TL2ParseSize(r); err != nil {
		return r, err
	}
...
func BuiltinVectorBenchmarksVrutoyTopLevelUnionInternalReadTL2(r []byte, vec *[]BenchmarksVrutoyTopLevelUnion) (_ []byte, err error) {
	...
	if cap(*vec) < elementCount {
		*vec = make([]BenchmarksVrutoyTopLevelUnion, elementCount)
	}
