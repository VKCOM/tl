...
func (item *CasesTL2TestVectorBool) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorBoolCalculateLayout(sizes, true, &item.X); sz != 0 {
		currentSize += sz
...
func (item *CasesTL2TestVectorBool) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if w, sizes, sz = BuiltinVectorBoolInternalWriteTL2(w, sizes, true, &item.X); sz != 0 {
		currentBlock |= 2
	}
...
func (item *CasesTL2TestVectorBool) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesTL2TestVectorBool) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, ErrorInvalidUnionIndex("casesTL2.testVectorBool", index)
		}
	}
	if block&2 != 0 {
		if currentR, err = BuiltinVectorBoolInternalReadTL2(currentR, &item.X); err != nil {
			return currentR, err
		}
	} else {
		item.X = item.X[:0]
...
func (item *CasesTL2TestVectorBool) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
