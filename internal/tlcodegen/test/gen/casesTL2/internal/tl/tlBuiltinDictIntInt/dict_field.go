...
func BuiltinDictIntIntFillRandom(rg *basictl.RandGenerator, m *map[int32]int32) {
	...
	*m = make(map[int32]int32, l)
	for i := 0; i < int(l); i++ {
		var elem tlDictFieldIntInt.DictFieldIntInt
		elem.FillRandom(rg)
...
func BuiltinDictIntIntWrite(w []byte, m map[int32]int32) []byte {
	...
	for _, key := range keys {
		val := m[key]
		elem := tlDictFieldIntInt.DictFieldIntInt{Key: key, Value: val}
		w = elem.Write(w)
	}
	return w
}
...
func BuiltinDictIntIntCalculateLayout(sizes []int, optimizeEmpty bool, m *map[int32]int32) ([]int, int) {
	...
	for _, key := range keys {
		elem := tlDictFieldIntInt.DictFieldIntInt{Key: key, Value: (*m)[key]}
...
func BuiltinDictIntIntInternalReadTL2(r []byte, m *map[int32]int32) (_ []byte, err error) {
	...
	for i := 0; i < elementCount; i++ {
		elem := tlDictFieldIntInt.DictFieldIntInt{}
...
func BuiltinDictIntIntBytesFillRandom(rg *basictl.RandGenerator, vec *[]tlDictFieldIntInt.DictFieldIntInt) {
	rg.IncreaseDepth()
	l := basictl.RandomSize(rg)
	*vec = make([]tlDictFieldIntInt.DictFieldIntInt, l)
	for i := range *vec {
		(*vec)[i].FillRandom(rg)
	}
	rg.DecreaseDepth()
...
func BuiltinDictIntIntBytesRead(w []byte, vec *[]tlDictFieldIntInt.DictFieldIntInt) (_ []byte, err error) {
	var l uint32
	if w, err = basictl.NatRead(w, &l); err != nil {
		return w, err
	}
...
func BuiltinDictIntIntBytesWrite(w []byte, vec []tlDictFieldIntInt.DictFieldIntInt) []byte {
...
func BuiltinDictIntIntBytesCalculateLayout(sizes []int, optimizeEmpty bool, vec *[]tlDictFieldIntInt.DictFieldIntInt) ([]int, int) {
	if len(*vec) == 0 {
		if optimizeEmpty {
			return sizes, 0
		}
...
func BuiltinDictIntIntBytesInternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool, vec *[]tlDictFieldIntInt.DictFieldIntInt) ([]byte, []int, int) {
	if len(*vec) == 0 {
		if optimizeEmpty {
			return w, sizes, 0
		}
...
func BuiltinDictIntIntBytesInternalReadTL2(r []byte, vec *[]tlDictFieldIntInt.DictFieldIntInt) (_ []byte, err error) {
	currentSize := 0
	if r, currentSize, err = basictl.TL2ParseSize(r); err != nil {
		return r, err
	}
	...
	if cap(*vec) < elementCount {
		*vec = make([]tlDictFieldIntInt.DictFieldIntInt, elementCount)
	}
...
func BuiltinDictIntIntBytesReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer, vec *[]tlDictFieldIntInt.DictFieldIntInt) error {
	*vec = (*vec)[:cap(*vec)]
	index := 0
	if in != nil {
		in.Delim('{')
		if !in.Ok() {
			return internal.ErrorInvalidJSON("[]tlDictFieldIntInt.DictFieldIntInt", "expected json object")
		}
		for ; !in.IsDelim('}'); index++ {
			if len(*vec) <= index {
				var newValue tlDictFieldIntInt.DictFieldIntInt
				*vec = append(*vec, newValue)
				*vec = (*vec)[:cap(*vec)]
			}
			keyBytes := []byte(in.UnsafeFieldName(false))
			if !in.Ok() {
				return internal.ErrorInvalidJSON("[]tlDictFieldIntInt.DictFieldIntInt", "expected correct json value in key")
			}
			in2 := basictl.JsonLexer{Data: keyBytes}
		...
		in.Delim('}')
		if !in.Ok() {
			return internal.ErrorInvalidJSON("[]tlDictFieldIntInt.DictFieldIntInt", "expected json object's end")
		}
	}
	*vec = (*vec)[:index]
	return nil
...
func BuiltinDictIntIntBytesWriteJSON(w []byte, vec []tlDictFieldIntInt.DictFieldIntInt) []byte {
	tctx := basictl.JSONWriteContext{}
	return BuiltinDictIntIntBytesWriteJSONOpt(&tctx, w, vec)
}
func BuiltinDictIntIntBytesWriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, vec []tlDictFieldIntInt.DictFieldIntInt) []byte {
	w = append(w, '{')
	for _, elem := range vec {
		key := elem.Key
		w = basictl.JSONAddCommaIfNeeded(w)
