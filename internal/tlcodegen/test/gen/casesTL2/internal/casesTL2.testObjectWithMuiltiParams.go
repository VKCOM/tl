...
func (item *CasesTL2TestObjectWithMuiltiParams) FillRandom(rg *basictl.RandGenerator) {
	...
	if basictl.RandomInt(rg)&1 != 0 {
		item.tl2mask0 |= 1
		BuiltinVectorIntFillRandom(rg, &item.F1)
	} else {
		item.F1 = item.F1[:0]
	}
	if basictl.RandomInt(rg)&1 != 0 {
		item.tl2mask0 |= 2
		BuiltinVectorIntFillRandom(rg, &item.F2)
	} else {
		item.F2 = item.F2[:0]
	}
}
...
func (item *CasesTL2TestObjectWithMuiltiParams) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if item.tl2mask0&1 != 0 {
		sizes, sz = BuiltinVectorIntCalculateLayout(sizes, false, &item.F1)
		currentSize += sz
		lastUsedByte = currentSize
	}
	if item.tl2mask0&2 != 0 {
		sizes, sz = BuiltinVectorIntCalculateLayout(sizes, false, &item.F2)
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTL2TestObjectWithMuiltiParams) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if item.tl2mask0&1 != 0 {
		w, sizes, _ = BuiltinVectorIntInternalWriteTL2(w, sizes, false, &item.F1)
		currentBlock |= 2
	}
	if item.tl2mask0&2 != 0 {
		w, sizes, _ = BuiltinVectorIntInternalWriteTL2(w, sizes, false, &item.F2)
		currentBlock |= 4
	}
...
func (item *CasesTL2TestObjectWithMuiltiParams) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		item.tl2mask0 |= 1
		if currentR, err = BuiltinVectorIntInternalReadTL2(currentR, &item.F1); err != nil {
			return currentR, err
		}
	} else {
		item.F1 = item.F1[:0]
	...
	if block&4 != 0 {
		item.tl2mask0 |= 2
		if currentR, err = BuiltinVectorIntInternalReadTL2(currentR, &item.F2); err != nil {
			return currentR, err
		}
	} else {
		item.F2 = item.F2[:0]
