...
func (item *CasesTL2TestFunctionWithDep1) ReadResultJSON(jctx *basictl.JSONReadContext, in *basictl.JsonLexer, ret *[]int32) error {
	if err := BuiltinTupleIntReadJSONGeneral(jctx, in, ret, item.N); err != nil {
		return err
	}
	return nil
}
...
func (item *CasesTL2TestFunctionWithDep1) writeResultJSON(jctx *basictl.JSONWriteContext, w []byte, ret []int32) (_ []byte, err error) {
	if w, err = BuiltinTupleIntWriteJSONOpt(jctx, w, ret, item.N); err != nil {
		return w, err
	}
	return w, nil
}
...
func (item *CasesTL2TestFunctionWithDep1) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
	w = append(w, '{')
	backupIndexN := len(w)
