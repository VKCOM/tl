...
func (item *CasesTL2TestObjectWithParam4) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinTuple4IntCalculateLayout(sizes, true, &item.Y); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTL2TestObjectWithParam4) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = BuiltinTuple4IntInternalWriteTL2(w, sizes, true, &item.Y); sz != 0 {
		currentBlock |= 4
	}
...
func (item *CasesTL2TestObjectWithParam4) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesTL2TestObjectWithParam4) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&4 != 0 {
		if currentR, err = BuiltinTuple4IntInternalReadTL2(currentR, &item.Y); err != nil {
			return currentR, err
		}
	} else {
		BuiltinTuple4IntReset(&item.Y)
...
func (item *CasesTL2TestObjectWithParam4) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
