...
func (item *CasesBytesTestDictInt) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorDictionaryAnyFieldIntIntCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
...
func (item *CasesBytesTestDictInt) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if w, sizes, sz = BuiltinVectorDictionaryAnyFieldIntIntInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
...
func (item *CasesBytesTestDictInt) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = BuiltinVectorDictionaryAnyFieldIntIntInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		BuiltinVectorDictionaryAnyFieldIntIntReset(item.Dict)
...
func (item *CasesBytesTestDictIntBytes) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorDictionaryAnyFieldIntIntBytesCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesBytesTestDictIntBytes) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	currentBlockPosition := len(w)
	w = append(w, 0)
	if w, sizes, sz = BuiltinVectorDictionaryAnyFieldIntIntBytesInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
...
func (item *CasesBytesTestDictIntBytes) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = BuiltinVectorDictionaryAnyFieldIntIntBytesInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		item.Dict = item.Dict[:0]
