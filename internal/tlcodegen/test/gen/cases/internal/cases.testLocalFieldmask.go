...
func (item *CasesTestLocalFieldmask) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if !propF2Presented && item.F1&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
	if !propF3Presented && item.F2&(1<<1) != 0 {
		item.tl2mask0 |= 2
	}
	if !propF4Presented && item.F2&(1<<1) != 0 {
		item.tl2mask0 |= 4
	}
	if propF2Presented {
		item.F1 |= 1 << 0
...
func (item *CasesTestLocalFieldmask) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesTestLocalFieldmask) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
