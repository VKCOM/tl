...
func (item *CasesTestInplaceStructArgs2) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if propArgPresented {
		...
		if err := item.Arg.ReadJSONGeneral(jctx, inArg, item.A1, item.A2, item.A3, item.A3, item.A2); err != nil {
			return err
		}
	}
	if !propArgPresented {
		if err := item.Arg.ReadJSONGeneral(jctx, nil, item.A1, item.A2, item.A3, item.A3, item.A2); err != nil {
			return err
		}
	}
	return nil
...
func (item *CasesTestInplaceStructArgs2) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
	w = append(w, '{')
	backupIndexA1 := len(w)
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"arg":`...)
	if w, err = item.Arg.WriteJSONOpt(jctx, w, item.A1, item.A2, item.A3, item.A3, item.A2); err != nil {
		return w, err
	}
	return append(w, '}'), nil
}
...
func (item *CasesTestInplaceStructArgs2) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesTestInplaceStructArgs2) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
