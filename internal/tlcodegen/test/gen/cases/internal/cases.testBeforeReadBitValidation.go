...
func (item CasesTestBeforeReadBitValidation) RepairMasksValue() CasesTestBeforeReadBitValidation {
	item.RepairMasks()
	return item
}
...
func (item *CasesTestBeforeReadBitValidation) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if !propAPresented && item.N&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
	if !propBPresented && item.N&(1<<1) != 0 {
		item.tl2mask0 |= 2
	}
	if propAPresented {
		item.N |= 1 << 0
