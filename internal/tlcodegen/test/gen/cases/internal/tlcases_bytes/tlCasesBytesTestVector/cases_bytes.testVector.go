...
func (item *CasesBytesTestVector) ReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.Arr
	if block&(1<<1) != 0 {
		if currentR, err = tlBuiltinVectorString.BuiltinVectorStringReadTL2(currentR, &item.Arr); err != nil {
			return currentR, err
		}
	} else {
		item.Arr = item.Arr[:0]
...
func (item *CasesBytesTestVectorBytes) ReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.Arr
	if block&(1<<1) != 0 {
		if currentR, err = tlBuiltinVectorString.BuiltinVectorStringBytesReadTL2(currentR, &item.Arr); err != nil {
			return currentR, err
		}
	} else {
		item.Arr = item.Arr[:0]
