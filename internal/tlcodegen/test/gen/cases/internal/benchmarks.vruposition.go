...
func (item *BenchmarksVruposition) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if !propCommitBitPresented && item.FieldsMask&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
	if !propMetaBlockPresented && item.FieldsMask&(1<<1) != 0 {
		item.tl2mask0 |= 2
	}
	if !propSplitPayloadPresented && item.FieldsMask&(1<<3) != 0 {
		item.tl2mask0 |= 4
	}
	if !propRotationBlockPresented && item.FieldsMask&(1<<5) != 0 {
		item.tl2mask0 |= 8
	}
	if !propCanonicalHashPresented && item.FieldsMask&(1<<15) != 0 {
		item.tl2mask0 |= 16
	}
	if !propSeqNumberPresented && item.FieldsMask&(1<<14) != 0 {
		item.tl2mask0 |= 32
	}
	if trueTypeCommitBitValue {
		item.FieldsMask |= 1 << 0
...
func (item *BenchmarksVruposition) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *BenchmarksVruposition) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
