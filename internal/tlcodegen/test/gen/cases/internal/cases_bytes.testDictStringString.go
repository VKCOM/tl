...
func (item *CasesBytesTestDictStringString) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinDictStringStringCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
...
func (item *CasesBytesTestDictStringString) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if w, sizes, sz = BuiltinDictStringStringInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
...
func (item *CasesBytesTestDictStringString) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CasesBytesTestDictStringString) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = BuiltinDictStringStringInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		BuiltinDictStringStringReset(item.Dict)
...
func (item *CasesBytesTestDictStringString) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
...
func (item *CasesBytesTestDictStringStringBytes) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinDictStringStringBytesCalculateLayout(sizes, true, &item.Dict); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesBytesTestDictStringStringBytes) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	currentBlockPosition := len(w)
	w = append(w, 0)
	if w, sizes, sz = BuiltinDictStringStringBytesInternalWriteTL2(w, sizes, true, &item.Dict); sz != 0 {
		currentBlock |= 2
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
...
func (item *CasesBytesTestDictStringStringBytes) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
	var sizes, sizes2 []int
	if tctx != nil {
		sizes = tctx.SizeBuffer[:0]
	}
...
func (item *CasesBytesTestDictStringStringBytes) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = BuiltinDictStringStringBytesInternalReadTL2(currentR, &item.Dict); err != nil {
			return currentR, err
		}
	} else {
		item.Dict = item.Dict[:0]
...
func (item *CasesBytesTestDictStringStringBytes) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
	return item.InternalReadTL2(r)
}
