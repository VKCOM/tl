...
func (item *CasesTestBeforeReadBitValidation) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if item.tl2mask0&1 != 0 {
		sizes, sz = tlBuiltinTupleInt.BuiltinTupleIntCalculateLayout(sizes, false, &item.A)
		currentSize += sz
		lastUsedByte = currentSize
	}
	if item.tl2mask0&2 != 0 {
		sizes, sz = tlBuiltinTupleInt.BuiltinTupleIntCalculateLayout(sizes, false, &item.B)
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *CasesTestBeforeReadBitValidation) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if item.tl2mask0&1 != 0 {
		w, sizes, _ = tlBuiltinTupleInt.BuiltinTupleIntInternalWriteTL2(w, sizes, false, &item.A)
		currentBlock |= 4
	}
	if item.tl2mask0&2 != 0 {
		w, sizes, _ = tlBuiltinTupleInt.BuiltinTupleIntInternalWriteTL2(w, sizes, false, &item.B)
		currentBlock |= 8
	}
