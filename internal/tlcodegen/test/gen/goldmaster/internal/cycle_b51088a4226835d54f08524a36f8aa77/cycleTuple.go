...
func (item *CycleTuple) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "c":
				...
				propCPresented = true
				if err := tlBuiltinTuple3Int.BuiltinTuple3IntReadJSONGeneral(jctx, in, &item.C); err != nil {
					return err
				}
				item.tl2mask0 |= 2
			default:
	...
	if !propCPresented && item.N&(1<<2) != 0 {
		item.tl2mask0 |= 2
	}
	if propAPresented {
		item.N |= 1 << 0
...
func (item *CycleTuple) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
	...
	if item.tl2mask0&2 != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"c":`...)
		w = tlBuiltinTuple3Int.BuiltinTuple3IntWriteJSONOpt(jctx, w, &item.C)
	}
	return append(w, '}'), nil
}
...
func (item *CycleTuple) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *CycleTuple) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
