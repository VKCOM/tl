...
func (item *UsefulServiceGetUserEntityResult) ReadTL1(w []byte, nat_fields_mask uint32) (_ []byte, err error) {
	if w, err = basictl.StringRead(w, &item.EntityId); err != nil {
		return w, err
	}
	return item.PaymentItems.ReadTL1Boxed(w, nat_fields_mask)
}

func (item *UsefulServiceGetUserEntityResult) WriteTL1(w []byte, nat_fields_mask uint32) []byte {
	w = basictl.StringWrite(w, item.EntityId)
	w = item.PaymentItems.WriteTL1Boxed(w, nat_fields_mask)
	return w
}

func (item *UsefulServiceGetUserEntityResult) ReadTL1Boxed(w []byte, nat_fields_mask uint32) (_ []byte, err error) {
...
func (item *UsefulServiceGetUserEntityResult) WriteTL1Boxed(w []byte, nat_fields_mask uint32) []byte {
	w = basictl.NatWrite(w, 0xe494698a)
	return item.WriteTL1(w, nat_fields_mask)
}

func (item *UsefulServiceGetUserEntityResult) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer, nat_fields_mask uint32) error {
	var propEntityIdPresented bool
	var propPaymentItemsPresented bool
	if in != nil {
		in.Delim('{')
		if !in.Ok() {
			return in.Error()
		...
		for !in.IsDelim('}') {
			...
			case "payment_items":
				if propPaymentItemsPresented {
					return internal.ErrorInvalidJSONWithDuplicatingKeys("usefulService.getUserEntityResult", "payment_items")
				}
				propPaymentItemsPresented = true
				if err := item.PaymentItems.ReadJSONGeneral(jctx, in, nat_fields_mask); err != nil {
					return err
				}
			default:
	...
	if !propPaymentItemsPresented {
		if err := item.PaymentItems.ReadJSONGeneral(jctx, nil, nat_fields_mask); err != nil {
			return err
		}
	}
	return nil
...
func (item *UsefulServiceGetUserEntityResult) WriteJSONGeneral(jctx *basictl.JSONWriteContext, w []byte, nat_fields_mask uint32) (_ []byte, err error) {
...
func (item *UsefulServiceGetUserEntityResult) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte, nat_fields_mask uint32) []byte {
	w = append(w, '{')
	...
	w = item.PaymentItems.WriteJSONOpt(jctx, w, nat_fields_mask)
...
func (item *UsefulServiceGetUserEntityResult) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *UsefulServiceGetUserEntityResult) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
