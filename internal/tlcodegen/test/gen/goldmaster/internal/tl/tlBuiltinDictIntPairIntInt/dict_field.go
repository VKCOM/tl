...
package tlBuiltinDictIntPairIntInt
...
func BuiltinDictIntPairIntIntReset(m map[int32]tlPairIntInt.PairIntInt) {
	clear(m)
}

func BuiltinDictIntPairIntIntFillRandom(rg *basictl.RandGenerator, m *map[int32]tlPairIntInt.PairIntInt) {
	rg.IncreaseDepth()
	l := basictl.RandomSize(rg)
	*m = make(map[int32]tlPairIntInt.PairIntInt, l)
	for i := 0; i < int(l); i++ {
		var elem tlDictFieldIntPairIntInt.DictFieldIntPairIntInt
		elem.FillRandom(rg)
		(*m)[elem.Key] = elem.Value
	}
	rg.DecreaseDepth()
}
func BuiltinDictIntPairIntIntReadTL1(w []byte, m *map[int32]tlPairIntInt.PairIntInt) (_ []byte, err error) {
	var l uint32
...
func BuiltinDictIntPairIntIntWriteTL1(w []byte, m map[int32]tlPairIntInt.PairIntInt) []byte {
	...
	for _, key := range keys {
		val := m[key]
		elem := tlDictFieldIntPairIntInt.DictFieldIntPairIntInt{Key: key, Value: val}
		w = elem.WriteTL1(w)
	}
	return w
}

func BuiltinDictIntPairIntIntCalculateLayout(sizes []int, optimizeEmpty bool, m *map[int32]tlPairIntInt.PairIntInt) ([]int, int) {
	if len(*m) == 0 {
		if optimizeEmpty {
			return sizes, 0
		}
	...
	for _, key := range keys {
		elem := tlDictFieldIntPairIntInt.DictFieldIntPairIntInt{Key: key, Value: (*m)[key]}
...
func BuiltinDictIntPairIntIntInternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool, m *map[int32]tlPairIntInt.PairIntInt) ([]byte, []int, int) {
	if len(*m) == 0 {
		if optimizeEmpty {
			return w, sizes, 0
		}
...
func BuiltinDictIntPairIntIntInternalReadTL2(r []byte, m *map[int32]tlPairIntInt.PairIntInt) (_ []byte, err error) {
	currentSize := 0
	...
	for i := 0; i < elementCount; i++ {
		elem := tlDictFieldIntPairIntInt.DictFieldIntPairIntInt{}
