...
func (item *Replace10) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
	if w, err = tlBuiltinTupleTuple3Replace10Elem.BuiltinTupleTuple3Replace10ElemWriteJSONOpt(tctx, w, item.A, item.N, item.N); err != nil {
		return w, err
	}
...
func (item *Replace10) CalculateLayout(sizes []int) []int {
	...
	if len(item.A) != 0 {
		sizes = tlBuiltinTupleTuple3Replace10Elem.BuiltinTupleTuple3Replace10ElemCalculateLayout(sizes, &item.A, item.N, item.N)
...
func (item *Replace10) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = tlBuiltinTupleTuple3Replace10Elem.BuiltinTupleTuple3Replace10ElemInternalWriteTL2(w, sizes, &item.A, item.N, item.N)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace10) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.A
	if block&(1<<2) != 0 {
		if currentR, err = tlBuiltinTupleTuple3Replace10Elem.BuiltinTupleTuple3Replace10ElemInternalReadTL2(currentR, &item.A, item.N, item.N); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
