...
func (item *Replace18) CalculateLayout(sizes []int) []int {
	...
	if len(item.A) != 0 {
		sizes = tlBuiltinVectorVectorVectorInt.BuiltinVectorVectorVectorIntCalculateLayout(sizes, &item.A)
...
func (item *Replace18) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 1)
			w, sizes = tlBuiltinVectorVectorVectorInt.BuiltinVectorVectorVectorIntInternalWriteTL2(w, sizes, &item.A)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace18) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.A
	if block&(1<<1) != 0 {
		if currentR, err = tlBuiltinVectorVectorVectorInt.BuiltinVectorVectorVectorIntInternalReadTL2(currentR, &item.A); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
