...
func (item *Replace15) CalculateLayout(sizes []int) []int {
	...
	if len(item.A) != 0 {
		sizes = tlBuiltinTupleReplace15Elem.BuiltinTupleReplace15ElemCalculateLayout(sizes, &item.A, item.N, item.N)
	...
	currentPosition = len(sizes)
	if len(item.B) != 0 {
		sizes = tlBuiltinTupleReplace15Elem1.BuiltinTupleReplace15Elem1CalculateLayout(sizes, &item.B, item.N, item.N)
		if sizes[currentPosition] != 0 {
			lastUsedByte = 1
			currentSize += sizes[currentPosition]
			currentSize += basictl.TL2CalculateSize(sizes[currentPosition])
	...
	currentPosition = len(sizes)
	if len(item.C) != 0 {
		sizes = tlBuiltinTupleReplace15Elem2.BuiltinTupleReplace15Elem2CalculateLayout(sizes, &item.C, item.N, item.N)
		if sizes[currentPosition] != 0 {
			lastUsedByte = 1
			currentSize += sizes[currentPosition]
			currentSize += basictl.TL2CalculateSize(sizes[currentPosition])
...
func (item *Replace15) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = tlBuiltinTupleReplace15Elem.BuiltinTupleReplace15ElemInternalWriteTL2(w, sizes, &item.A, item.N, item.N)
		} else {
			sizes = sizes[1:]
		}
	}
	...
	if len(item.B) != 0 {
		...
		if sizes[0] != 0 {
			serializedSize += basictl.TL2CalculateSize(sizes[0])
			currentBlock |= (1 << 3)
			w, sizes = tlBuiltinTupleReplace15Elem1.BuiltinTupleReplace15Elem1InternalWriteTL2(w, sizes, &item.B, item.N, item.N)
		} else {
			sizes = sizes[1:]
		}
	}
	...
	if len(item.C) != 0 {
		...
		if sizes[0] != 0 {
			serializedSize += basictl.TL2CalculateSize(sizes[0])
			currentBlock |= (1 << 4)
			w, sizes = tlBuiltinTupleReplace15Elem2.BuiltinTupleReplace15Elem2InternalWriteTL2(w, sizes, &item.C, item.N, item.N)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace15) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.A
	if block&(1<<2) != 0 {
		if currentR, err = tlBuiltinTupleReplace15Elem.BuiltinTupleReplace15ElemInternalReadTL2(currentR, &item.A, item.N, item.N); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
	...
	// read item.B
	if block&(1<<3) != 0 {
		if currentR, err = tlBuiltinTupleReplace15Elem1.BuiltinTupleReplace15Elem1InternalReadTL2(currentR, &item.B, item.N, item.N); err != nil {
			return currentR, err
		}
	} else {
		item.B = item.B[:0]
	...
	// read item.C
	if block&(1<<4) != 0 {
		if currentR, err = tlBuiltinTupleReplace15Elem2.BuiltinTupleReplace15Elem2InternalReadTL2(currentR, &item.C, item.N, item.N); err != nil {
			return currentR, err
		}
	} else {
		item.C = item.C[:0]
