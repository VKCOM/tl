...
package tlBuiltinDictIntTupleString
...
func BuiltinDictIntTupleStringFillRandom(rg *basictl.RandGenerator, m *map[int32][]string, nat_t uint32) {
	...
	*m = make(map[int32][]string, l)
	for i := 0; i < int(l); i++ {
		var elem tlDictFieldIntTupleString.DictFieldIntTupleString
...
func BuiltinDictIntTupleStringWriteTL1(w []byte, m map[int32][]string, nat_t uint32) (_ []byte, err error) {
	...
	for _, key := range keys {
		val := m[key]
		elem := tlDictFieldIntTupleString.DictFieldIntTupleString{Key: key, Value: val}
...
func BuiltinDictIntTupleStringCalculateLayout(sizes []int, optimizeEmpty bool, m *map[int32][]string) ([]int, int) {
	...
	for _, key := range keys {
		elem := tlDictFieldIntTupleString.DictFieldIntTupleString{Key: key, Value: (*m)[key]}
...
func BuiltinDictIntTupleStringInternalReadTL2(r []byte, m *map[int32][]string) (_ []byte, err error) {
	...
	for i := 0; i < elementCount; i++ {
		elem := tlDictFieldIntTupleString.DictFieldIntTupleString{}
