...
func (item *TupleString) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer, nat_n uint32) error {
	if err := tlBuiltinTupleString.BuiltinTupleStringReadJSONGeneral(jctx, in, item.ptr(), nat_n); err != nil {
		return err
	}
	return nil
}
...
func (item *TupleString) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
	if w, err = tlBuiltinTupleString.BuiltinTupleStringWriteJSONOpt(jctx, w, *item.ptr(), nat_n); err != nil {
		return w, err
	}
	return w, nil
}
