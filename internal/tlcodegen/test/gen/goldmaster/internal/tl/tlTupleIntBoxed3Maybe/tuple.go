...
func (item *TupleIntBoxed3Maybe) ReadTL1Boxed(w []byte) (_ []byte, err error) {
	...
	if item.Ok {
		return tlBuiltinTuple3IntBoxed.BuiltinTuple3IntBoxedReadTL1(w, &item.Value)
	}
	return w, nil
}
...
func (item *TupleIntBoxed3Maybe) WriteTL1Boxed(w []byte) []byte {
	if item.Ok {
		...
		return tlBuiltinTuple3IntBoxed.BuiltinTuple3IntBoxedWriteTL1(w, &item.Value)
	}
...
func (item *TupleIntBoxed3Maybe) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinTuple3IntBoxed.BuiltinTuple3IntBoxedCalculateLayout(sizes, true, &item.Value); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *TupleIntBoxed3Maybe) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	currentBlock |= 1
	if w, sizes, sz = tlBuiltinTuple3IntBoxed.BuiltinTuple3IntBoxedInternalWriteTL2(w, sizes, true, &item.Value); sz != 0 {
		currentBlock |= 2
	}
...
func (item *TupleIntBoxed3Maybe) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&2 != 0 {
		if currentR, err = tlBuiltinTuple3IntBoxed.BuiltinTuple3IntBoxedInternalReadTL2(currentR, &item.Value); err != nil {
			return currentR, err
		}
	} else {
