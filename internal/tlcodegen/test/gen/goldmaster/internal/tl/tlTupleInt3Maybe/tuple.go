...
func (item *TupleInt3Maybe) CalculateLayout(sizes []int) []int {
	...
	if item.Ok {
		...
		sizes = tlBuiltinTuple3Int.BuiltinTuple3IntCalculateLayout(sizes, &item.Value)
...
func (item *TupleInt3Maybe) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if item.Ok {
		...
		if sizes[0] != 0 {
			...
			w, sizes = tlBuiltinTuple3Int.BuiltinTuple3IntInternalWriteTL2(w, sizes, &item.Value)
		} else {
			sizes = sizes[1:]
		}
	}
