...
func (item *UseTrue) ReadTL1(w []byte) (_ []byte, err error) {
	...
	if item.Fm&(1<<2) != 0 {
		item.tl2mask0 |= 4
		if w, err = tlBool.BoolReadTL1Boxed(w, &item.E); err != nil {
			return w, err
		}
	} else {
		item.E = false
...
func (item *UseTrue) WriteTL1(w []byte) []byte {
	...
	if item.Fm&(1<<2) != 0 {
		w = tlBool.BoolWriteTL1Boxed(w, item.E)
	}
	return w
}
...
func (item *UseTrue) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if !propAPresented && item.Fm&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
	if !propBPresented && item.Fm&(1<<1) != 0 {
		item.tl2mask0 |= 2
	}
	if !propEPresented && item.Fm&(1<<2) != 0 {
		item.tl2mask0 |= 4
	}
	if trueTypeAValue {
		item.Fm |= 1 << 0
...
func (item *UseTrue) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *UseTrue) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
