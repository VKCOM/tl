...
package tlReplace14Long

import (
	"github.com/vkcom/tl/internal/tlcodegen/test/gen/goldmaster/internal"
	"github.com/vkcom/tl/internal/tlcodegen/test/gen/goldmaster/internal/tl/tlBuiltinTupleReplace14ElemLong"
...
type Replace14Long struct {
	K uint32
	A []tlReplace14ElemLong.Replace14ElemLong
}
...
func (item *Replace14Long) FillRandom(rg *basictl.RandGenerator, nat_n uint32) {
	...
	tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongFillRandom(rg, &item.A, item.K, nat_n, item.K)
}
...
func (item *Replace14Long) Read(w []byte, nat_n uint32) (_ []byte, err error) {
	...
	return tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongRead(w, &item.A, item.K, nat_n, item.K)
}
...
func (item *Replace14Long) Write(w []byte, nat_n uint32) (_ []byte, err error) {
	...
	if w, err = tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongWrite(w, item.A, item.K, nat_n, item.K); err != nil {
		return w, err
	}
	return w, nil
}
...
func (item *Replace14Long) ReadJSON(legacyTypeNames bool, in *basictl.JsonLexer, nat_n uint32) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "k":
				if propKPresented {
					return internal.ErrorInvalidJSONWithDuplicatingKeys("replace14", "k")
				}
			...
			case "a":
				if rawA != nil {
					return internal.ErrorInvalidJSONWithDuplicatingKeys("replace14", "a")
				}
				rawA = in.Raw()
				if !in.Ok() {
					return in.Error()
			...
			default:
				return internal.ErrorInvalidJSONExcessElement("replace14", key)
			}
			in.WantComma()
		}
		in.Delim('}')
	...
	if err := tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongReadJSON(legacyTypeNames, inAPointer, &item.A, item.K, nat_n, item.K); err != nil {
		return err
	}
...
func (item *Replace14Long) WriteJSONGeneral(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
...
func (item *Replace14Long) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
	w = append(w, '{')
	backupIndexK := len(w)
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
	if w, err = tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongWriteJSONOpt(tctx, w, item.A, item.K, nat_n, item.K); err != nil {
		return w, err
	}
...
func (item *Replace14Long) InternalWriteTL2(w []byte, sizes []int, nat_n uint32) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongInternalWriteTL2(w, sizes, &item.A, item.K, nat_n, item.K)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace14Long) WriteTL2(w []byte, ctx *basictl.TL2WriteContext, nat_n uint32) []byte {
	var sizes []int
	if ctx != nil {
...
func (item *Replace14Long) InternalReadTL2(r []byte, nat_n uint32) (_ []byte, err error) {
	currentSize := 0
	...
	// read item.A
	if block&(1<<2) != 0 {
		if currentR, err = tlBuiltinTupleReplace14ElemLong.BuiltinTupleReplace14ElemLongInternalReadTL2(currentR, &item.A, item.K, nat_n, item.K); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
...
func (item *Replace14Long) ReadTL2(r []byte, ctx *basictl.TL2ReadContext, nat_n uint32) (_ []byte, err error) {
