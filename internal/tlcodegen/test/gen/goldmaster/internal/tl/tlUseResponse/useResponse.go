...
func (item *UseResponse) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = tlBuiltinTupleAbResponse.BuiltinTupleAbResponseCalculateLayout(sizes, true, &item.X); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *UseResponse) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = tlBuiltinTupleAbResponse.BuiltinTupleAbResponseInternalWriteTL2(w, sizes, true, &item.X); sz != 0 {
		currentBlock |= 4
	}
