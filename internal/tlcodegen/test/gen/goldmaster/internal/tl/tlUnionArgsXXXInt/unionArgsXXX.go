...
func (item *UnionArgsXXX1Int) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer, nat_Y uint32) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "x":
				...
				propXPresented = true
				if err := tlBuiltinTupleInt.BuiltinTupleIntReadJSONGeneral(tctx, in, &item.X, nat_Y); err != nil {
					return err
				}
			default:
	...
	if !propXPresented {
		if err := tlBuiltinTupleInt.BuiltinTupleIntReadJSONGeneral(tctx, nil, &item.X, nat_Y); err != nil {
			return err
		}
	}
	return nil
