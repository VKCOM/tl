...
func (item *Replace13ElemLong) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32, nat_k uint32) (_ []byte, err error) {
	...
	if nat_n&(1<<0) != 0 {
		...
		w = append(w, `"b":`...)
		if w, err = tlBuiltinTuplePairBoxedIntLong.BuiltinTuplePairBoxedIntLongWriteJSONOpt(tctx, w, item.B, nat_k); err != nil {
			return w, err
		}
	}
	if nat_k&(1<<0) != 0 {
...
func (item *Replace13ElemLong) InternalReadTL2(r []byte, nat_n uint32, nat_k uint32) (_ []byte, err error) {
	...
	if block&(1<<1) != 0 {
		if nat_n&(1<<0) != 0 {
			if currentR, err = tlBuiltinTuplePairBoxedIntLong.BuiltinTuplePairBoxedIntLongInternalReadTL2(currentR, &item.B, nat_k); err != nil {
				return currentR, err
			}
		} else {
