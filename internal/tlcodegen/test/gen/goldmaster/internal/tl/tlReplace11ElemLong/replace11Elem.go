...
func (item *Replace11ElemLong) WriteJSONGeneral(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32, nat_k uint32) (_ []byte, err error) {
...
func (item *Replace11ElemLong) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32, nat_k uint32) (_ []byte, err error) {
	w = append(w, '{')
	if nat_n&(1<<0) != 0 {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = append(w, `"b":`...)
		w = tlBuiltinTuple3PairBoxedIntLong.BuiltinTuple3PairBoxedIntLongWriteJSONOpt(tctx, w, &item.B)
	}
	backupIndexC := len(w)
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"c":`...)
	if w, err = tlBuiltinTupleInt.BuiltinTupleIntWriteJSONOpt(tctx, w, item.C, nat_n); err != nil {
		return w, err
	}
...
func (item *Replace11ElemLong) InternalReadTL2(r []byte, nat_n uint32, nat_k uint32) (_ []byte, err error) {
	...
	if block&(1<<1) != 0 {
		if nat_n&(1<<0) != 0 {
			if currentR, err = tlBuiltinTuple3PairBoxedIntLong.BuiltinTuple3PairBoxedIntLongInternalReadTL2(currentR, &item.B); err != nil {
				return currentR, err
			}
		} else {
	...
	// read item.C
	if block&(1<<2) != 0 {
		if currentR, err = tlBuiltinTupleInt.BuiltinTupleIntInternalReadTL2(currentR, &item.C, nat_n); err != nil {
			return currentR, err
		}
	} else {
		item.C = item.C[:0]
