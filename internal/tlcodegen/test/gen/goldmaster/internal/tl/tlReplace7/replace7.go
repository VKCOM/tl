...
func (item *Replace7) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
	if w, err = tlBuiltinTupleTupleInt.BuiltinTupleTupleIntWriteJSONOpt(tctx, w, item.A, item.N, item.M); err != nil {
		return w, err
	}
...
func (item *Replace7) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.A
	if block&(1<<3) != 0 {
		if currentR, err = tlBuiltinTupleTupleInt.BuiltinTupleTupleIntInternalReadTL2(currentR, &item.A, item.N, item.M); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
