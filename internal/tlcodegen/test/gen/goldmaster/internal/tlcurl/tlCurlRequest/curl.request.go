...
func (item *CurlRequest) Read(w []byte) (_ []byte, err error) {
	...
	if item.FieldMask&(1<<0) != 0 {
		item.tl2mask0 |= 1
		if w, err = tlBuiltinDictStringString.BuiltinDictStringStringRead(w, &item.Headers); err != nil {
			return w, err
		}
	} else {
		tlBuiltinDictStringString.BuiltinDictStringStringReset(item.Headers)
...
func (item *CurlRequest) Write(w []byte) []byte {
	...
	if item.FieldMask&(1<<0) != 0 {
		w = tlBuiltinDictStringString.BuiltinDictStringStringWrite(w, item.Headers)
	}
	if item.FieldMask&(1<<1) != 0 {
