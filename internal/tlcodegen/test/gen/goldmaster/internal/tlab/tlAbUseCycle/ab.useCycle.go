...
func (item AbUseCycle) RepairMasksValue() AbUseCycle {
	item.RepairMasks()
	return item
}
func (item *AbUseCycle) RepairMasks() {
	item.A.RepairMasks()
}
...
func (item *AbUseCycle) ReadTL1(w []byte) (_ []byte, err error) {
	...
	return tlBuiltinVectorAColor.BuiltinVectorAColorReadTL1(w, &item.B)
}
...
func (item *AbUseCycle) WriteTL1(w []byte) []byte {
	w = item.A.WriteTL1(w)
	w = tlBuiltinVectorAColor.BuiltinVectorAColorWriteTL1(w, item.B)
	return w
}
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "b":
				...
				propBPresented = true
				if err := tlBuiltinVectorAColor.BuiltinVectorAColorReadJSONGeneral(jctx, in, &item.B); err != nil {
					return err
				}
			default:
...
func (item *AbUseCycle) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"b":`...)
	w = tlBuiltinVectorAColor.BuiltinVectorAColorWriteJSONOpt(jctx, w, item.B)
	if !(len(item.B) != 0) {
		w = w[:backupIndexB]
	}
	return append(w, '}')
...
func (item *AbUseCycle) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	sizes = append(sizes, 260427869)
	...
	if sizes, sz = tlBuiltinVectorAColor.BuiltinVectorAColorCalculateLayout(sizes, true, &item.B); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *AbUseCycle) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	if sizes[0] != 260427869 {
	...
	if w, sizes, sz = tlBuiltinVectorAColor.BuiltinVectorAColorInternalWriteTL2(w, sizes, true, &item.B); sz != 0 {
		currentBlock |= 4
	}
...
func (item *AbUseCycle) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *AbUseCycle) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
