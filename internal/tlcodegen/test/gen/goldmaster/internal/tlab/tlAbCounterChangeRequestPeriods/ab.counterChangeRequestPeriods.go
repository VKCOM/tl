...
func (item *AbCounterChangeRequestPeriodsMany) ReadTL1(w []byte) (_ []byte, err error) {
	return tlBuiltinVectorInt.BuiltinVectorIntReadTL1(w, &item.ObjectsPeridos)
}
...
func (item *AbCounterChangeRequestPeriodsMany) WriteTL1(w []byte) []byte {
	w = tlBuiltinVectorInt.BuiltinVectorIntWriteTL1(w, item.ObjectsPeridos)
	return w
}
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "objects_peridos":
				...
				if err := tlBuiltinVectorInt.BuiltinVectorIntReadJSONGeneral(jctx, in, &item.ObjectsPeridos); err != nil {
					return err
				}
			default:
...
func (item *AbCounterChangeRequestPeriodsMany) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	...
	w = tlBuiltinVectorInt.BuiltinVectorIntWriteJSONOpt(jctx, w, item.ObjectsPeridos)
