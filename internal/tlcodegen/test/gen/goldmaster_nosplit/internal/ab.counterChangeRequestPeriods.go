...
func (item *AbCounterChangeRequestPeriods) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	case "ab.counterChangeRequestPeriodsMany#14a35d80", "ab.counterChangeRequestPeriodsMany", "#14a35d80":
		if tctx.IsTL2 && _tag != "ab.counterChangeRequestPeriodsMany" {
			return ErrorInvalidUnionLegacyTagJSON("ab.CounterChangeRequestPeriods", _tag)
		}
...
func (item *AbCounterChangeRequestPeriodsMany) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorIntCalculateLayout(sizes, true, &item.ObjectsPeridos); sz != 0 {
		currentSize += sz
...
func (item *AbCounterChangeRequestPeriodsMany) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	w = append(w, 0)
	if w, sizes, sz = BuiltinVectorIntInternalWriteTL2(w, sizes, true, &item.ObjectsPeridos); sz != 0 {
		currentBlock |= 2
	}
...
func (item *AbCounterChangeRequestPeriodsMany) InternalReadTL2(r []byte, block byte) (_ []byte, err error) {
	currentR := r
	if block&2 != 0 {
		if currentR, err = BuiltinVectorIntInternalReadTL2(currentR, &item.ObjectsPeridos); err != nil {
			return currentR, err
		}
	} else {
		item.ObjectsPeridos = item.ObjectsPeridos[:0]
