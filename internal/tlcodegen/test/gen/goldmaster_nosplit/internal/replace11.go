...
func (item *Replace11Long) FillRandom(rg *basictl.RandGenerator, nat_n uint32) {
	...
	BuiltinTupleReplace11ElemLongFillRandom(rg, &item.A, item.K, nat_n, item.K)
}
...
func (item *Replace11Long) Write(w []byte, nat_n uint32) (_ []byte, err error) {
	...
	if w, err = BuiltinTupleReplace11ElemLongWrite(w, item.A, item.K, nat_n, item.K); err != nil {
		return w, err
	}
	return w, nil
}
...
func (item *Replace11Long) ReadJSON(legacyTypeNames bool, in *basictl.JsonLexer, nat_n uint32) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "k":
				if propKPresented {
					return ErrorInvalidJSONWithDuplicatingKeys("replace11", "k")
				}
			...
			case "a":
				if rawA != nil {
					return ErrorInvalidJSONWithDuplicatingKeys("replace11", "a")
				}
				rawA = in.Raw()
				if !in.Ok() {
					return in.Error()
			...
			default:
				return ErrorInvalidJSONExcessElement("replace11", key)
			}
			in.WantComma()
		}
		in.Delim('}')
	...
	if err := BuiltinTupleReplace11ElemLongReadJSON(legacyTypeNames, inAPointer, &item.A, item.K, nat_n, item.K); err != nil {
		return err
	}
...
func (item *Replace11Long) WriteJSONGeneral(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
...
func (item *Replace11Long) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
	w = append(w, '{')
	backupIndexK := len(w)
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
	if w, err = BuiltinTupleReplace11ElemLongWriteJSONOpt(tctx, w, item.A, item.K, nat_n, item.K); err != nil {
		return w, err
	}
...
func (item *Replace11Long) CalculateLayout(sizes []int, nat_n uint32) []int {
	...
	if len(item.A) != 0 {
		sizes = BuiltinTupleReplace11ElemLongCalculateLayout(sizes, &item.A, item.K, nat_n, item.K)
...
func (item *Replace11Long) InternalWriteTL2(w []byte, sizes []int, nat_n uint32) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = BuiltinTupleReplace11ElemLongInternalWriteTL2(w, sizes, &item.A, item.K, nat_n, item.K)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace11Long) WriteTL2(w []byte, ctx *basictl.TL2WriteContext, nat_n uint32) []byte {
	var sizes []int
	if ctx != nil {
...
func (item *Replace11Long) ReadTL2(r []byte, ctx *basictl.TL2ReadContext, nat_n uint32) (_ []byte, err error) {
