...
func (item *Replace18) WriteTL2(w []byte, ctx *basictl.TL2WriteContext) []byte {
	var sizes []int
	if ctx != nil {
...
func (item *Replace18) InternalReadTL2(r []byte) (_ []byte, err error) {
	currentSize := 0
	...
	// read item.A
	if block&(1<<1) != 0 {
		if currentR, err = BuiltinVectorVectorVectorIntInternalReadTL2(currentR, &item.A); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
...
func (item *Replace18) ReadTL2(r []byte, ctx *basictl.TL2ReadContext) (_ []byte, err error) {
