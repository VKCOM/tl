...
func (item *Service5Insert) writeResultJSON(tctx *basictl.JSONWriteContext, w []byte, ret Service5Output) (_ []byte, err error) {
...
func (item *Service5Insert) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if propPersistentPresented && !trueTypePersistentValue && (item.Flags&(1<<0) != 0) {
		return ErrorInvalidJSON("service5.insert", "field 'persistent' is explicitly set to false, but corresponding fieldmask item.Flags bit 0 is 1")
	}
	if item.Flags&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
