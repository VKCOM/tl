...
func (item *Service5InsertList) WriteResultTL2(w []byte, ctx *basictl.TL2WriteContext, ret ListService5Output) []byte {
...
func (item *Service5InsertList) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if propPersistentPresented && !trueTypePersistentValue && (item.Flags&(1<<0) != 0) {
		return ErrorInvalidJSON("service5.insertList", "field 'persistent' is explicitly set to false, but corresponding fieldmask item.Flags bit 0 is 1")
	}
	if item.Flags&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
