...
func (item *Replace4) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer, nat_n uint32) error {
	...
	if !propAPresented {
		if err := BuiltinTupleIntReadJSONGeneral(tctx, nil, &item.A, nat_n); err != nil {
			return err
		}
	}
	return nil
...
// This method is general version of WriteJSON, use it instead!
func (item *Replace43) WriteJSONGeneral(tctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
...
func (item *Replace43) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
