...
func (item *Replace7) FillRandom(rg *basictl.RandGenerator) {
	...
	BuiltinTupleTupleIntFillRandom(rg, &item.A, item.N, item.M)
}
...
func (item *Replace7) Read(w []byte) (_ []byte, err error) {
	...
	return BuiltinTupleTupleIntRead(w, &item.A, item.N, item.M)
}
...
func (item *Replace7) Write(w []byte) (_ []byte, err error) {
	...
	if w, err = BuiltinTupleTupleIntWrite(w, item.A, item.N, item.M); err != nil {
		return w, err
	}
	return w, nil
}
...
func (item *Replace7) ReadJSON(legacyTypeNames bool, in *basictl.JsonLexer) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "n":
				if propNPresented {
					return ErrorInvalidJSONWithDuplicatingKeys("replace7", "n")
				}
			...
			case "m":
				if propMPresented {
					return ErrorInvalidJSONWithDuplicatingKeys("replace7", "m")
				}
			...
			case "a":
				if rawA != nil {
					return ErrorInvalidJSONWithDuplicatingKeys("replace7", "a")
				}
				rawA = in.Raw()
				if !in.Ok() {
					return in.Error()
			...
			default:
				return ErrorInvalidJSONExcessElement("replace7", key)
			}
			in.WantComma()
		}
		in.Delim('}')
	...
	if err := BuiltinTupleTupleIntReadJSON(legacyTypeNames, inAPointer, &item.A, item.N, item.M); err != nil {
		return err
	}
...
func (item *Replace7) UnmarshalJSON(b []byte) error {
	if err := item.ReadJSON(true, &basictl.JsonLexer{Data: b}); err != nil {
		return ErrorInvalidJSON("replace7", err.Error())
	}
	return nil
}
...
func (item *Replace7) CalculateLayout(sizes []int) []int {
	...
	if len(item.A) != 0 {
		sizes = BuiltinTupleTupleIntCalculateLayout(sizes, &item.A, item.N, item.M)
...
func (item *Replace7) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.A) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 3)
			w, sizes = BuiltinTupleTupleIntInternalWriteTL2(w, sizes, &item.A, item.N, item.M)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *Replace7) WriteTL2(w []byte, ctx *basictl.TL2WriteContext) []byte {
	var sizes []int
	if ctx != nil {
...
func (item *Replace7) InternalReadTL2(r []byte) (_ []byte, err error) {
	currentSize := 0
	...
	// read item.A
	if block&(1<<3) != 0 {
		if currentR, err = BuiltinTupleTupleIntInternalReadTL2(currentR, &item.A, item.N, item.M); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
...
func (item *Replace7) ReadTL2(r []byte, ctx *basictl.TL2ReadContext) (_ []byte, err error) {
