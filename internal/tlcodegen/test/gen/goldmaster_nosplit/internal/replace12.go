...
func (item *Replace12) WriteTL2(w []byte, sizes []int) ([]byte, []int) {
	sizes = item.CalculateLayout(sizes[:0])
	w, _ = item.InternalWriteTL2(w, sizes)
	return w, sizes[:0]
}

func (item *Replace12) ReadTL2(r []byte) (_ []byte, err error) {
	currentSize := 0
	...
	// read item.A
	if block&(1<<2) != 0 {
		if currentR, err = BuiltinTupleTuple3Replace12ElemReadTL2(currentR, &item.A, item.N, item.N); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
