...
func (item *AbTopLevel1) ReadTL1(w []byte) (_ []byte, err error) {
...
func (item *AbTopLevel1) WriteTL1General(w []byte) (_ []byte, err error) {
	return item.WriteTL1(w), nil
}

func (item *AbTopLevel1) WriteTL1(w []byte) []byte {
	w = item.A.WriteTL1(w)
	w = item.B.WriteTL1(w)
	return w
}

func (item *AbTopLevel1) ReadTL1Boxed(w []byte) (_ []byte, err error) {
...
func (item *AbTopLevel1) WriteTL1BoxedGeneral(w []byte) (_ []byte, err error) {
	return item.WriteTL1Boxed(w), nil
}

func (item *AbTopLevel1) WriteTL1Boxed(w []byte) []byte {
...
func (item *AbTopLevel1) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if in != nil {
		...
		for !in.IsDelim('}') {
			...
			case "a":
				if propAPresented {
					return ErrorInvalidJSONWithDuplicatingKeys("ab.topLevel1", "a")
				}
				propAPresented = true
			...
			case "b":
				if propBPresented {
					return ErrorInvalidJSONWithDuplicatingKeys("ab.topLevel1", "b")
				}
				propBPresented = true
			...
			default:
				return ErrorInvalidJSONExcessElement("ab.topLevel1", key)
			}
			in.WantComma()
		}
		in.Delim('}')
...
func (item *AbTopLevel1) WriteJSONGeneral(jctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
...
func (item *AbTopLevel1) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
...
func (item *AbTopLevel1) UnmarshalJSON(b []byte) error {
	if err := item.ReadJSON(true, &basictl.JsonLexer{Data: b}); err != nil {
		return ErrorInvalidJSON("ab.topLevel1", err.Error())
	}
	return nil
}

func (item *AbTopLevel1) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	sizes = append(sizes, 1946049438)
...
func (item *AbTopLevel1) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
...
func (item *AbTopLevel1) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, ErrorInvalidUnionIndex("ab.topLevel1", index)
		}
	}
	if block&2 != 0 {
...
func (item *AbTopLevel1) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
