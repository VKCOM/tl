...
func BuiltinTuple3Replace12ElemInternalReadTL2(r []byte, vec *[3]Replace12Elem, nat_t uint32) (_ []byte, err error) {
	currentSize := 0
...
func BuiltinTuple3Replace12ElemWriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, vec *[3]Replace12Elem, nat_t uint32) []byte {
	w = append(w, '[')
	for _, elem := range *vec {
...
func BuiltinTupleTuple3Replace12ElemCalculateLayout(sizes []int, vec *[][3]Replace12Elem, nat_n uint32, nat_t uint32) []int {
	...
	for i := lastIndex; i < nat_n; i++ {
		var elem [3]Replace12Elem
		currentPosition := len(sizes)
		sizes = BuiltinTuple3Replace12ElemCalculateLayout(sizes, &elem, nat_t)
		currentSize += sizes[currentPosition]
		currentSize += basictl.TL2CalculateSize(sizes[currentPosition])
	}
...
func BuiltinTupleTuple3Replace12ElemInternalWriteTL2(w []byte, sizes []int, vec *[][3]Replace12Elem, nat_n uint32, nat_t uint32) ([]byte, []int) {
	...
	for i := lastIndex; i < nat_n; i++ {
		var elem [3]Replace12Elem
		w, sizes = BuiltinTuple3Replace12ElemInternalWriteTL2(w, sizes, &elem, nat_t)
	}
	return w, sizes
}

func BuiltinTupleTuple3Replace12ElemInternalReadTL2(r []byte, vec *[][3]Replace12Elem, nat_n uint32, nat_t uint32) (_ []byte, err error) {
	currentSize := 0
	if r, currentSize, err = basictl.TL2ParseSize(r); err != nil {
		return r, err
	}
	...
	for i := uint32(0); i < lastIndex; i++ {
		if currentR, err = BuiltinTuple3Replace12ElemInternalReadTL2(currentR, &(*vec)[i], nat_t); err != nil {
			return currentR, err
		}
	}
	...
	// reset elements if received less elements
	for i := lastIndex; i < nat_n; i++ {
		BuiltinTuple3Replace12ElemReset(&(*vec)[i])
	}
...
func BuiltinTupleTuple3Replace12ElemWriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, vec [][3]Replace12Elem, nat_n uint32, nat_t uint32) (_ []byte, err error) {
	if uint32(len(vec)) != nat_n {
		return w, ErrorWrongSequenceLength("[][3]Replace12Elem", len(vec), nat_n)
	}
	w = append(w, '[')
	for _, elem := range vec {
		w = basictl.JSONAddCommaIfNeeded(w)
		w = BuiltinTuple3Replace12ElemWriteJSONOpt(tctx, w, &elem, nat_t)
	}
	return append(w, ']'), nil
}
...
func (item *Replace12Elem) WriteTL2(w []byte, ctx *basictl.TL2WriteContext, nat_n uint32) []byte {
	var sizes []int
	if ctx != nil {
...
func (item *Replace12Elem) ReadTL2(r []byte, ctx *basictl.TL2ReadContext, nat_n uint32) (_ []byte, err error) {
