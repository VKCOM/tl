...
func (item *AbCall2) ReadResultTL1(w []byte, ret *CdTypeB) (_ []byte, err error) {
	return ret.ReadTL1Boxed(w)
}

func (item *AbCall2) WriteResultTL1(w []byte, ret CdTypeB) (_ []byte, err error) {
	w = ret.WriteTL1Boxed(w)
	return w, nil
}

func (item *AbCall2) ReadResultTL2(r []byte, tctx *basictl.TL2ReadContext, ret *CdTypeB) (_ []byte, err error) {
	currentSize := 0
...
func (item *AbCall2) calculateLayoutResult(sizes []int, optimizeEmpty bool, ret CdTypeB) ([]int, int) {
	sizes = append(sizes, 1427922651)
...
func (item *AbCall2) writeResultTL2(w []byte, sizes []int, optimizeEmpty bool, ret CdTypeB) ([]byte, []int, int) {
	if sizes[0] != 1427922651 {
...
func (item *AbCall2) WriteResultTL2(w []byte, tctx *basictl.TL2WriteContext, ret CdTypeB) []byte {
...
func (item *AbCall2) ReadResultJSON(jctx *basictl.JSONReadContext, in *basictl.JsonLexer, ret *CdTypeB) error {
...
func (item *AbCall2) writeResultJSON(jctx *basictl.JSONWriteContext, w []byte, ret CdTypeB) (_ []byte, err error) {
...
func (item *AbCall2) ReadResultTL1WriteResultJSON(jctx *basictl.JSONWriteContext, r []byte, w []byte) (_ []byte, _ []byte, err error) {
	var ret CdTypeB
...
func (item *AbCall2) ReadResultTL2WriteResultJSON(tctx *basictl.TL2ReadContext, jctx *basictl.JSONWriteContext, r []byte, w []byte) (_ []byte, _ []byte, err error) {
	var ret CdTypeB
	if r, err = item.ReadResultTL2(r, tctx, &ret); err != nil {
		return r, w, err
	}
...
func (item *AbCall2) ReadJSONGeneral(jctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
...
func (item *AbCall2) WriteJSONGeneral(jctx *basictl.JSONWriteContext, w []byte) (_ []byte, err error) {
...
func (item *AbCall2) WriteJSONOpt(jctx *basictl.JSONWriteContext, w []byte) []byte {
	w = append(w, '{')
...
func (item *AbCall2) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	sizes = append(sizes, 1427922651)
	sizePosition := len(sizes)
	sizes = append(sizes, 0)
...
func (item *AbCall2) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	if sizes[0] != 1427922651 {
		panic("tl2: tag mismatch between calculate and write")
	}
	currentSize := sizes[1]
...
func (item *AbCall2) WriteTL2(w []byte, tctx *basictl.TL2WriteContext) []byte {
	var sizes, sizes2 []int
	if tctx != nil {
		sizes = tctx.SizeBuffer[:0]
	}
...
func (item *AbCall2) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, ErrorInvalidUnionIndex("ab.call2", index)
		}
	}
	if block&2 != 0 {
...
func (item *AbCall2) ReadTL2(r []byte, tctx *basictl.TL2ReadContext) (_ []byte, err error) {
