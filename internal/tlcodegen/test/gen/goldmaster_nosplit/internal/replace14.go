...
func (item *Replace14Long) WriteJSONOpt(tctx *basictl.JSONWriteContext, w []byte, nat_n uint32) (_ []byte, err error) {
	...
	w = basictl.JSONAddCommaIfNeeded(w)
	w = append(w, `"a":`...)
	if w, err = BuiltinTupleReplace14ElemLongWriteJSONOpt(tctx, w, item.A, item.K, nat_n, item.K); err != nil {
		return w, err
	}
...
func (item *Replace14Long) InternalReadTL2(r []byte, nat_n uint32) (_ []byte, err error) {
	...
	// read item.A
	if block&(1<<2) != 0 {
		if currentR, err = BuiltinTupleReplace14ElemLongInternalReadTL2(currentR, &item.A, item.K, nat_n, item.K); err != nil {
			return currentR, err
		}
	} else {
		item.A = item.A[:0]
