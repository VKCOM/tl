...
func BuiltinTupleReplace15ElemInternalWriteTL2(w []byte, sizes []int, vec *[]Replace15Elem, nat_n uint32, nat_t uint32) ([]byte, []int) {
	currentSize := sizes[0]
	sizes = sizes[1:]
...
func BuiltinTupleReplace15ElemReadTL2(r []byte, vec *[]Replace15Elem, nat_n uint32, nat_t uint32) (_ []byte, err error) {
	saveR := r
	currentSize := 0
...
func (item *Replace15Elem) CalculateLayout(sizes []int, nat_n uint32) []int {
	sizePosition := len(sizes)
	sizes = append(sizes, 0)
...
func (item *Replace15Elem) InternalWriteTL2(w []byte, sizes []int, nat_n uint32) ([]byte, []int) {
	currentSize := sizes[0]
	sizes = sizes[1:]
...
func (item *Replace15Elem) ReadTL2(r []byte, nat_n uint32) (_ []byte, err error) {
	saveR := r
	currentSize := 0
	if r, currentSize, err = basictl.TL2ParseSize(r); err != nil {
		return r, err
	...
	if currentSize == 0 {
		...
		// read item.Y
		if block&(1<<2) != 0 {
			if r, err = basictl.IntRead(r, &item.Y); err != nil {
				return r, err
			}
		} else {
			item.Y = 0
