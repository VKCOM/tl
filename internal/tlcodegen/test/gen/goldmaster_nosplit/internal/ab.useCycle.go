...
func (item *AbUseCycle) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorAColorCalculateLayout(sizes, true, &item.B); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *AbUseCycle) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = BuiltinVectorAColorInternalWriteTL2(w, sizes, true, &item.B); sz != 0 {
		currentBlock |= 4
	}
...
func (item *AbUseCycle) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&1 != 0 {
		...
		if index != 0 {
			return r, ErrorInvalidUnionIndex("ab.useCycle", index)
		}
	}
	if block&2 != 0 {
	...
	if block&4 != 0 {
		if currentR, err = BuiltinVectorAColorInternalReadTL2(currentR, &item.B); err != nil {
			return currentR, err
		}
	} else {
		item.B = item.B[:0]
