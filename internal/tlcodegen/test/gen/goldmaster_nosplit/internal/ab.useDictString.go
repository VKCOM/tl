...
func (item *AbUseDictString) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.Tags) != 0 {
		...
		if sizes[0] != 0 {
			...
			currentBlock |= (1 << 2)
			w, sizes = BuiltinVectorDictionaryFieldStringInternalWriteTL2(w, sizes, &item.Tags)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *AbUseDictString) ReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.Tags
	if block&(1<<2) != 0 {
		if currentR, err = BuiltinVectorDictionaryFieldStringReadTL2(currentR, &item.Tags); err != nil {
			return currentR, err
		}
	} else {
		BuiltinVectorDictionaryFieldStringReset(item.Tags)
...
func (item *AbUseDictStringBytes) InternalWriteTL2(w []byte, sizes []int) ([]byte, []int) {
	...
	if len(item.Tags) != 0 {
		...
		if sizes[0] != 0 {
			serializedSize += basictl.TL2CalculateSize(sizes[0])
			currentBlock |= (1 << 2)
			w, sizes = BuiltinVectorDictionaryFieldStringBytesInternalWriteTL2(w, sizes, &item.Tags)
		} else {
			sizes = sizes[1:]
		}
	}
...
func (item *AbUseDictStringBytes) ReadTL2(r []byte) (_ []byte, err error) {
	...
	// read item.Tags
	if block&(1<<2) != 0 {
		if currentR, err = BuiltinVectorDictionaryFieldStringBytesReadTL2(currentR, &item.Tags); err != nil {
			return currentR, err
		}
	} else {
		item.Tags = item.Tags[:0]
