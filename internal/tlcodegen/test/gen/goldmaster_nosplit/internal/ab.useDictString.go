...
func (item *AbUseDictStringBytes) CalculateLayout(sizes []int, optimizeEmpty bool) ([]int, int) {
	...
	if sizes, sz = BuiltinVectorDictionaryFieldStringBytesCalculateLayout(sizes, true, &item.Tags); sz != 0 {
		currentSize += sz
		lastUsedByte = currentSize
	}
...
func (item *AbUseDictStringBytes) InternalWriteTL2(w []byte, sizes []int, optimizeEmpty bool) ([]byte, []int, int) {
	...
	if w, sizes, sz = BuiltinVectorDictionaryFieldStringBytesInternalWriteTL2(w, sizes, true, &item.Tags); sz != 0 {
		currentBlock |= 4
	}
	if currentBlockPosition < len(w) {
		w[currentBlockPosition] = currentBlock
...
func (item *AbUseDictStringBytes) InternalReadTL2(r []byte) (_ []byte, err error) {
	...
	if block&4 != 0 {
		if currentR, err = BuiltinVectorDictionaryFieldStringBytesInternalReadTL2(currentR, &item.Tags); err != nil {
			return currentR, err
		}
	} else {
		item.Tags = item.Tags[:0]
