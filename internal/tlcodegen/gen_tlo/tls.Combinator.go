...
func (item *TlsCombinator) Read(w []byte) (_ []byte, err error) {
	...
	if w, err = IntRead(w, &item.TypeName); err != nil {
		return w, err
	}
...
func (item *TlsCombinator) ReadBoxed(w []byte) (_ []byte, err error) {
	if w, err = NatReadExactTag(w, 0x5c0a1ed5); err != nil {
		return w, err
	}
	return item.Read(w)
}
...
func (item *TlsCombinator) WriteJSON(w []byte) (_ []byte, err error) {
	...
	if item.TypeName != 0 {
		...
		w = JSONWriteInt32(w, item.TypeName)
	}
	w = JSONAddCommaIfNeeded(w)
	w = append(w, `"left":`...)
	if w, err = item.Left.WriteJSON(w); err != nil {
		return w, err
	}
	w = JSONAddCommaIfNeeded(w)
	w = append(w, `"right":`...)
	if w, err = item.Right.WriteJSON(w); err != nil {
		return w, err
	}
	return append(w, '}'), nil
}
...
func (item *TlsCombinatorUnion) WriteBoxed(w []byte) (_ []byte, err error) {
	w = NatWrite(w, _TlsCombinatorUnion[item.index].TLTag)
	switch item.index {
	case 0:
...
func (item *TlsCombinatorUnion) WriteJSON(w []byte) (_ []byte, err error) {
	...
	case 0:
		...
		if w, err = item.valueCombinator.WriteJSON(w); err != nil {
			return w, err
		}
		return append(w, '}'), nil
	case 1:
...
func (item *TlsCombinatorV4) ReadBoxed(w []byte) (_ []byte, err error) {
	if w, err = NatReadExactTag(w, 0xe91692d5); err != nil {
		return w, err
	}
	return item.Read(w)
}
