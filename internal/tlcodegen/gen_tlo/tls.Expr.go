...
func (item *TlsExprNat) WriteJSON(w []byte) (_ []byte, err error) {
	...
	w = append(w, `"expr":`...)
	if w, err = item.Expr.WriteJSON(w); err != nil {
		return w, err
	}
	return append(w, '}'), nil
}
...
func (item *TlsExprType) ReadBoxed(w []byte) (_ []byte, err error) {
	if w, err = NatReadExactTag(w, 0xecc9da78); err != nil {
		return w, err
	}
	return item.Read(w)
}
...
func (item *TlsExprUnion) WriteBoxed(w []byte) (_ []byte, err error) {
	w = NatWrite(w, _TlsExprUnion[item.index].TLTag)
	switch item.index {
	case 0:
...
func (item *TlsExprUnion) WriteJSON(w []byte) (_ []byte, err error) {
	...
	case 0:
		...
		if w, err = item.valueType.WriteJSON(w); err != nil {
			return w, err
		}
		return append(w, '}'), nil
	case 1:
		w = append(w, `{"type":"tls.exprNat#dcb49bd8","value":`...)
		if w, err = item.valueNat.WriteJSON(w); err != nil {
			return w, err
		}
		return append(w, '}'), nil
	default: // Impossible due to panic above
