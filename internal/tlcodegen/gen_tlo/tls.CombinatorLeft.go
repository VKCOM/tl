...
func (item *TlsCombinatorLeft) ReadBoxed(w []byte) (_ []byte, err error) {
	if w, err = NatReadExactTag(w, 0x4c12c6d9); err != nil {
		return w, err
	}
	return item.Read(w)
}
...
func (item *TlsCombinatorLeftBuiltin) ReadBoxed(w []byte) ([]byte, error) {
	return NatReadExactTag(w, 0xcd211f63)
}
func (item *TlsCombinatorLeftBuiltin) WriteBoxed(w []byte) ([]byte, error) {
	return NatWrite(w, 0xcd211f63), nil
}
...
func (item *TlsCombinatorLeftUnion) WriteBoxed(w []byte) (_ []byte, err error) {
	w = NatWrite(w, _TlsCombinatorLeftUnion[item.index].TLTag)
	switch item.index {
	case 0:
		return w, nil
	case 1:
...
func (item *TlsCombinatorLeftUnion) WriteJSON(w []byte) (_ []byte, err error) {
	...
	case 1:
		...
		if w, err = item.valueCombinatorLeft.WriteJSON(w); err != nil {
			return w, err
		}
		return append(w, '}'), nil
	default: // Impossible due to panic above
