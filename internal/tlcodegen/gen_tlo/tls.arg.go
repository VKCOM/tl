...
func (item *TlsArg) Read(w []byte) (_ []byte, err error) {
	...
	if w, err = NatRead(w, &item.Flags); err != nil {
		return w, err
	}
	if item.Flags&(1<<1) != 0 {
	...
	if item.Flags&(1<<2) != 0 {
		if w, err = IntRead(w, &item.ExistVarNum); err != nil {
			return w, err
		}
	} else {
		item.ExistVarNum = 0
	...
	if item.Flags&(1<<2) != 0 {
		if w, err = IntRead(w, &item.ExistVarBit); err != nil {
			return w, err
		}
	} else {
		item.ExistVarBit = 0
...
func (item *TlsArg) Write(w []byte) (_ []byte, err error) {
	...
	w = NatWrite(w, item.Flags)
	if item.Flags&(1<<1) != 0 {
		w = IntWrite(w, item.VarNum)
	}
	if item.Flags&(1<<2) != 0 {
		w = IntWrite(w, item.ExistVarNum)
	}
	if item.Flags&(1<<2) != 0 {
		w = IntWrite(w, item.ExistVarBit)
	}
	return item.Type.WriteBoxed(w)
}

func (item *TlsArg) ReadBoxed(w []byte) (_ []byte, err error) {
	if w, err = NatReadExactTag(w, 0x29dfe61b); err != nil {
		return w, err
	}
	return item.Read(w)
}
...
func (item *TlsArg) WriteJSON(w []byte) (_ []byte, err error) {
	...
	if item.Flags != 0 {
		w = JSONAddCommaIfNeeded(w)
		w = append(w, `"flags":`...)
		w = JSONWriteUint32(w, item.Flags)
	}
	if item.Flags&(1<<1) != 0 {
		if item.VarNum != 0 {
			w = JSONAddCommaIfNeeded(w)
	...
	if item.Flags&(1<<2) != 0 {
		if item.ExistVarNum != 0 {
			...
			w = JSONWriteInt32(w, item.ExistVarNum)
		}
	}
	if item.Flags&(1<<2) != 0 {
		if item.ExistVarBit != 0 {
			...
			w = JSONWriteInt32(w, item.ExistVarBit)
		}
	}
	w = JSONAddCommaIfNeeded(w)
	w = append(w, `"type":`...)
	if w, err = item.Type.WriteJSON(w); err != nil {
		return w, err
	}
	return append(w, '}'), nil
}
