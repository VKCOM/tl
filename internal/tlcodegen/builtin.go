...
func ReplaceSquareBrackets(tl *tlast.TL) error {
	for typeIndex := 0; typeIndex < len(*tl); typeIndex++ { // We append anonymous types while iterating
		...
		for fieldIndex, field := range typ.Fields {
			...
			insideField := &field
			elementT := &field.FieldType
			for insideField.IsRepeated {
				if len(insideField.ScaleRepeat.Rep) == 0 {
					return insideField.ScaleRepeat.PR.BeautifulError(fmt.Errorf("repetition with no fields is not allowed"))
				}
				if !insideField.ScaleRepeat.ExplicitScale {
					endRange := tlast.PositionRange{Outer: insideField.ScaleRepeat.PR.Outer, Begin: insideField.ScaleRepeat.PR.Begin, End: insideField.ScaleRepeat.PR.Begin}
					if insideField != &field {
						return endRange.BeautifulError(fmt.Errorf("anonymous scale repeat can be used only in top-level square brackets"))
					}
					if fieldIndex == 0 { // Allow shortcut to last template parameters
						// hren {n:#} a:[int] = Hren;
						a := typ.TemplateArguments[len(typ.TemplateArguments)-1]
						...
						insideField.ScaleRepeat.Scale.Scale = a.FieldName
					} else {
						prevField := typ.Fields[fieldIndex-1]
						if prevField.FieldType.Type.String() != "#" {
							e1 := endRange.BeautifulError(fmt.Errorf("anonymous scale repeat references previous field %q which should have type #", prevField.FieldName))
							e2 := prevField.FieldType.PR.BeautifulError(fmt.Errorf("see here"))
							return tlast.BeautifulError2(e1, e2)
						...
						if prevField.FieldName == "" {
							newFields = newFields[:len(newFields)-1]
							// The only case when we replace 2 fields with vector
							// hren # a:[int] = Hren;
							insideField.IsRepeated = false
							elementT.Type = tlast.Name{Name: BuiltinVectorName}
							elementT.Bare = true
							elementT.Args = []tlast.ArithmeticOrType{{
								T: insideField.ScaleRepeat.Rep[0].FieldType,
							}}
							elementT = &elementT.Args[0].T
							insideField = &insideField.ScaleRepeat.Rep[0]
							continue
						}
						// hren n:# a:[int] = Hren;
						insideField.ScaleRepeat.Scale.Scale = prevField.FieldName
					}
					insideField.ScaleRepeat.ExplicitScale = true
					insideField.ScaleRepeat.Scale.IsArith = false
				}
				// TODO - wrong PR here
				rep := tlast.ArithmeticOrType{T: tlast.TypeRef{Type: tlast.Name{Name: insideField.ScaleRepeat.Scale.Scale}}}
				if insideField.ScaleRepeat.Scale.IsArith {
				...
				rep.T.PR = insideField.ScaleRepeat.Scale.PR
				insideField.IsRepeated = false
				if len(insideField.ScaleRepeat.Rep) == 1 && insideField.ScaleRepeat.Rep[0].Mask == nil {
					elementT.Type = tlast.Name{Name: BuiltinTupleName}
					elementT.Bare = true
					elementT.Args = []tlast.ArithmeticOrType{{
						T: insideField.ScaleRepeat.Rep[0].FieldType,
					}, // TODO - use outer brackets
						rep,
					}
					...
					//	Bare: true,
					//	Args: []tlast.ArithmeticOrType{
					//		{TypeRef: insideField.ScaleRepeat.Rep[0].FieldType},
					//		rep,
					//	},
					// }
					elementT = &elementT.Args[0].T
				...
				// Impossible name to prevent name collisions
				name := typ.Construct.Name.Name + "Elem"
				if elemCounter != 0 {
					name += strconv.Itoa(elemCounter)
				}
				elemCounter++
				zero := uint32(0)
				res := &tlast.Combinator{
					Construct: tlast.Constructor{Name: tlast.Name{
						Namespace: typ.Construct.Name.Namespace,
						Name:      name,
					}, ID: &zero},
					TemplateArguments: typ.TemplateArguments,
					Fields:            insideField.ScaleRepeat.Rep,
					TypeDecl: tlast.TypeDeclaration{Name: tlast.Name{
						Namespace: typ.Construct.Name.Namespace,
						Name:      strings.Title(name), //lint:ignore SA1019 "transfer to golang.org/x/text/cases in single PR; better idea?"
					}},
				}
				for _, f := range newFields {
					if f.FieldType.Type.String() == "#" && f.FieldName != "" {
				...
				tWithArgs := tlast.TypeRef{
					Type: tlast.Name{
						Namespace: typ.Construct.Name.Namespace,
						Name:      name, // ref to constructor will be bare
					},
				}
				for _, a := range res.TemplateArguments {
					tWithArgs.Args = append(tWithArgs.Args, tlast.ArithmeticOrType{T: tlast.TypeRef{
